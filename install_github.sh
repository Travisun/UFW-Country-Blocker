...
# UFW CIDR Blocker GitHub安装脚本
# 从GitHub直接下载并安装UFW CIDR Blocker
...
TEMP_DIR="/tmp/${SCRIPT_NAME}_install_$(date +%Y%m%d_%H%M%S)"
...
download_files() {
    ...
    local files=(
        ...
        "ufw_cidr_blocker.service"
        "ufw_cidr_blocker.timer"
    )
...
install_config_file() {
    ...
    if [ -f "ufw_cidr_blocker.conf" ]; then
        cp "ufw_cidr_blocker.conf" "$CONFIG_FILE"
        log "配置文件已安装到: $CONFIG_FILE"
    else
        error_exit "配置文件不存在: ufw_cidr_blocker.conf"
    fi
}
...
install_systemd_service() {
    ...
    # 复制服务文件
    if [ -f "ufw_cidr_blocker.service" ]; then
        cp "ufw_cidr_blocker.service" "$SERVICE_FILE"
        log "服务文件已安装到: $SERVICE_FILE"
    else
        error_exit "服务文件不存在: ufw_cidr_blocker.service"
    fi
    
    # 复制定时器文件
    if [ -f "ufw_cidr_blocker.timer" ]; then
        cp "ufw_cidr_blocker.timer" "$TIMER_FILE"
        log "定时器文件已安装到: $TIMER_FILE"
    else
        error_exit "定时器文件不存在: ufw_cidr_blocker.timer"
    fi
...
show_installation_info() {
    ...
    echo ""
    echo "卸载方法:"
    echo "  - 运行卸载脚本: sudo bash <(curl -fsSL $RAW_URL/uninstall.sh)"
    echo ""
}
...
# 主函数
main() {
    log "开始安装UFW CIDR Blocker (GitHub版本)"
...
# 错误处理
trap 'error_exit "安装过程中发生错误，请检查日志"' ERR
